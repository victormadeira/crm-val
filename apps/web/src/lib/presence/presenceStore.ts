...
// Em produção, estes dados são substituídos pelos eventos do socket.
// ─────────────────────────────────────────────────────────────────────────────
...
interface PresenceState {
  users: Record<string, UserPresence>;   // userId → UserPresence
  roomConfig: RoomConfig;
  selectedUserId: string | null;
  connected: boolean;
  useMock: boolean;

  // Ações internas (chamadas pelo socket hook)
...
export const usePresenceStore = create<PresenceState>((set, get) => ({
  // Inicia com dados mock para desenvolvimento
